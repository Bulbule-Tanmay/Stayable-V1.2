...
export default function EnquiriesPage({
  ...
  const localEnquiries: Enquiry[] = extraIds
    .map((id, i) => {
  ...
  const persistedEnquiries: Enquiry[] = enquiries.map((enquiry) => ({
    id: enquiry.id,

    listingId: enquiry.listing_id,

    listingName: enquiry.listings?.name ?? "Listing",

    sentAt: enquiry.created_at
      ? new Date(enquiry.created_at).toLocaleDateString("en-IN")
      : "Recently",

    status: enquiry.status === "closed" ? "pending" : enquiry.status,

    lastMessage:
      enquiry.message || "You requested a visit. Waiting for owner to confirm.",

    phone: enquiry.listings?.phone ?? "",

    waMessage: enquiry.listings?.wa_message ?? "",

    image: (enquiry.listings?.images ?? [])[0] ?? "",
  }))
  ...
  const allEnquiries = [
    ...
    ...localEnquiries.filter(
      (local) =>
        !persistedEnquiries.some((item) => item.listingId === local.listingId),
    ),
  ]
    ...
    <div className="flex flex-col w-full pb-28">
      <div className="px-4 py-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-on-surface-muted">
            {allEnquiries.length} active conversations
          </p>
        </div>
      ...
      <div className="mx-4 mb-4 bg-surface-mid rounded-2xl px-4 py-3 flex items-center gap-3">
        ...
        <p className="text-xs text-on-surface font-medium leading-relaxed">
          All enquiries are direct — no middlemen. Chat with owners directly on
          WhatsApp.
        </p>
      </div>
              ...
              <div className="flex gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <h4 className="font-display text-sm font-bold text-on-surface truncate flex-1">
                      {enq.listingName}
                    </h4>
                    <span

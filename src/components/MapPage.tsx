...
export default function MapPage({ savedIds, onSaveToggle, onViewDetails, onListView, listings }: Props) {
  ...
  return (
    <div className="flex flex-col w-full relative">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="font-display text-sm font-bold text-on-surface truncate">MIT-WPU Campus</span>
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-secondary text-white shrink-0">Center</span>
              </div>
              <p className="text-xs text-on-surface-muted truncate">Showing {listings.length} properties within 2 km</p>
            </div>
          </div>
      ...
      {/* Map canvas */}
      <div className="relative w-full h-[420px] overflow-hidden bg-surface-mid mx-3 rounded-2xl" style={{ width: "calc(100% - 1.5rem)" }}>
        {/* SVG map background */}
        <svg
        ...
        >
          ...
          <path d="M-20 40 Q 60 70 110 30 T 220 50 L 250 -20 L -30 -20 Z" fill="#D9ECD9" opacity="0.8" />
          <path d="M300 360 Q 340 320 410 340 L 420 440 L 280 430 Z" fill="#D9ECD9" opacity="0.7" />
          {/* Campus polygon */}
          <path d="M120 155 L 220 147 L 238 218 L 135 236 Z" fill="#2563EB" fillOpacity="0.12" />
          <path d="M120 155 L 220 147 L 238 218 L 135 236 Z" stroke="#0051D5" strokeWidth="2" strokeDasharray="4 3" opacity="0.6" />
          <text x="138" y="194" fill="#0051D5" fontFamily="Plus Jakarta Sans" fontSize="9" fontWeight="700" letterSpacing="1" opacity="0.85">MIT-WPU ZONE</text>
          {/* Roads */}
          <path d="M-20 108 C 100 118, 260 90, 420 65" stroke="#FFFFFF" strokeWidth="12" strokeLinecap="round" />
          <path d="M-20 108 C 100 118, 260 90, 420 65" stroke="#CBD5E1" strokeWidth="2" strokeLinecap="round" />
          <text x="30" y="100" fill="#76777D" fontFamily="Inter" fontSize="9" fontWeight="600" letterSpacing="0.5">PAUD ROAD</text>
          <path d="M178 60 L 168 290 L 110 420" stroke="#FFFFFF" strokeWidth="9" strokeLinecap="round" />
          <path d="M178 60 L 168 290 L 110 420" stroke="#CBD5E1" strokeWidth="1.5" strokeLinecap="round" />
          <path d="M30 228 C 110 228, 200 248, 420 240" stroke="#FFFFFF" strokeWidth="7" />
          ...
          <path d="M 128 272 L 148 272 L 158 225 L 172 210" stroke="#0051D5" strokeDasharray="5 4" strokeLinecap="round" strokeWidth="3.5" />
          <circle cx="128" cy="272" fill="#0051D5" r="4" />
          {/* Radius ring */}
          <circle cx="178" cy="192" r="130" fill="none" stroke="#2563EB" strokeDasharray="6 6" strokeWidth="1.5" opacity="0.35" />
        </svg>
          ...
          return (
            ...
            >
              <div className="relative flex flex-col items-center">
                ...
                >
                  {isSelected && <span className="w-2 h-2 rounded-full bg-verified animate-pulse" />}
                  ₹{(l.priceFrom ?? l.price_from ?? 0 / 1000).toFixed(1)}k
                  <span className={`text-[10px] font-normal ${isSelected ? "text-white/70" : "text-on-surface-muted"}`}>
                    {l.name.split(" ")[0]}
                  </span>
                </div>
                <div className={`w-2.5 h-2.5 rotate-45 -mt-1 ${isSelected ? "bg-primary-dark" : "bg-white"}`} />
              </div>
            </button>
          );
        })}
        ...
        <div className="bg-white rounded-2xl p-4 shadow-xl flex flex-col gap-3">
          <div className="flex gap-3 items-start">
            <div className="relative w-24 h-24 rounded-xl overflow-hidden shrink-0 bg-surface-mid">
              ...
              <div className="absolute top-1 left-1 bg-primary-dark/80 rounded-full px-1.5 py-0.5 flex items-center gap-0.5">
                ...
                <span className="text-[9px] text-white font-bold">{selected.images.length}</span>
              </div>
            </div>
            <div className="flex flex-col flex-1 min-w-0">
              <div className="flex items-center justify-between gap-1">
                <div className="flex items-center gap-1 bg-surface-low px-2 py-0.5 rounded-full">
                  <span className="material-symbols-outlined text-[13px] text-verified" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span>
                  <span className="text-[11px] text-verified font-semibold">Verified Listing</span>
                </div>
                <button
                ...
                >
                  <span
                    ...
                    style={{
                      fontVariationSettings: savedIds.has(selected.id) ? "'FILL' 1" : "'FILL' 0",
              ...
              <h2 className="font-display text-base font-bold text-on-surface mt-1 truncate">{selected.name}</h2>
              <p className="text-xs text-on-surface-muted truncate">{selected.address}</p>
              <div className="flex items-baseline gap-1 mt-1">
                <span className="font-display text-lg font-extrabold text-on-surface">
                  ₹{(selected.priceFrom ?? selected.price_from ?? 0).toLocaleString("en-IN")}
                </span>
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center gap-1 bg-surface-high px-2 py-1 rounded-full">
              ...
              <span className="text-[11px] font-semibold">{selected.distance}</span>
              <span className="text-on-surface-muted text-[10px]">• {selected.walkTime ?? selected.walk_time ?? ""}</span>
            </div>

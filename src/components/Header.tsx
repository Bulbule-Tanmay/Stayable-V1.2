...
export default function Header({ campus }: Props) {
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur-xl shadow-[0_1px_8px_rgba(15,23,42,0.06)]">
      <div className="h-16 px-3 flex items-center justify-between gap-2">
        ...
        <button className="flex items-center gap-1.5 h-11 px-3 bg-surface-low rounded-full transition-colors active:bg-surface-mid flex-1 max-w-[180px]">
          <span className="material-symbols-outlined text-secondary text-[18px]">school</span>
          <span className="text-xs font-semibold text-on-surface truncate">{campus}</span>

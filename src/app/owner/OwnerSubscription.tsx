...
export default function OwnerSubscription() {
  ...
  const handleSubscribe = async (plan: any) => {
    ...
    try {
      ...
      const { order_id, amount, currency } = await createRazorpayOrder(plan.id)

      const key_id = import.meta.env.VITE_RAZORPAY_KEY_ID

      if (!key_id) throw new Error("Payment is not configured.")
  ...
  const isCurrentPlan = (plan: any) =>
    current?.subscription_plans?.name === plan.name &&
    ...
    <div className="py-6 flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-on-surface-muted">
          Choose the right plan for your listing needs
        </p>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-emerald-700">
                ₹{current.subscription_plans?.price}/month • Renews{" "}
                {current.expires_at
                  ? new Date(current.expires_at).toLocaleDateString("en-IN", {
                      day: "numeric",
                      month: "long",
                      year: "numeric",
                    })
      ...
      {loading ? (
        <div className="flex flex-col gap-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-48 rounded-2xl bg-surface-high animate-pulse"
            />
          ))}
        </div>
      ) : plans.length === 0 ? (
        <div className="text-center py-12 text-on-surface-muted">
          ...
          <p className="text-xs mt-1">
            Please run the database migration to seed plans.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-on-surface-muted">
                        Up to {plan.max_listings} listing
                        {plan.max_listings > 1 ? "s" : ""}
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="font-display text-3xl font-extrabold text-primary-dark">
                        ₹{plan.price}
                      </span>
                      <p className="text-xs text-on-surface-muted">
                        /month + GST
                  ...
                  <ul className="flex flex-col gap-2">
                    {(Array.isArray(plan.features) ? plan.features : []).map(
                      (f: string) => (
                        ...
                        >
                          <span
                            className="material-symbols-outlined text-[16px] text-verified shrink-0"
                            style={{ fontVariationSettings: "'FILL' 1" }}
                          >
                            check_circle
                          </span>

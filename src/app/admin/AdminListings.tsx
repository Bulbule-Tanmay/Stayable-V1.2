...
export default function AdminListings() {
  ...
  const handleApprove = async (id: string, approved: boolean) => {
    ...
    try {
      const updated = await adminApproveListing(id, approved)
    ...
    <div className="py-6 flex flex-col gap-5">
      <div>
        ...
        <p className="text-sm text-white/40">
          {items.length} total listings across all owners
        </p>
      </div>
      {error && (
      ...
      <div className="relative">
        <span className="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-[18px] text-white/30">
          search
        </span>
        <input
          type="text"
          ...
          >
            ...
            <span
              className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                filter === f ? "bg-black/10" : "bg-white/10"
              }`}
            >
      ...
      {counts.pending > 0 && filter !== "approved" && (
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm font-semibold text-amber-300">
            {counts.pending} listing{counts.pending > 1 ? "s" : ""} awaiting
            review
          </p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          {filtered.map((l) => (
            <div
              key={l.id}
              className="bg-[#1a1d27] border border-white/5 rounded-2xl overflow-hidden"
            >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      ...
                      <p className="text-xs font-bold text-white/60">
                        ₹
                        {(l.priceFrom ?? l.price_from ?? 0).toLocaleString(
                          "en-IN",
                        )}
                        /mo
                      </p>
